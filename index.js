...
let guessCount = userStats.getItem("guess-count");
let gameBoard = userStats.getItem("game-board");
const firstTimer = () => {
...
const startGame = () => {
  ...
  timesPlayed++;
  userStats.setItem("guess-count", "0");
...
/* Select an answer from the above array */
let today = new Date()
let answerIndex = today.getDate() + today.getMonth() + today.getYear();
let answerWord = targetWords[answerIndex];

/* Initialize guess word and create an array from guess div tiles */
let guesses = Array.from(document.querySelectorAll("div.guess-block"));
let guessWord = "";
...
const statuses = {
  ...
  "wrong-place": "background-color: #c9b458; color: white;",
  "wrong": "background-color: #878a8c; color: white;"
}

/*Create functions changing block state and color */
const getBlockState = (block) => blockState = block.dataset.state;
const changeBlockColor = (block) => {
  getBlockState(block);
  block.style = statuses[blockState]
}
const flipBlock = (block) => block.classList.add("flip-horizontal-top")

/* Create button functions for inputing/deleting/submitting guesses */
let buttons = document.querySelectorAll("button");

const inputLetter = (block, letter) => {
  if(guessWord.length >= answerWord.length) return;
  block.innerHTML = letter;
  block.dataset.value = letter;
  block.dataset.state = "filled";
  guessWord += letter;
...
const deleteLetter = (block) => {
  if(guessWord.length > 0){
    block.innerHTML = "";
    block.dataset.value = "";
    block.dataset.state = "not-filled";
...
/*Create functions changing key state and color */
const getKeyState = (key) => keyState = key.dataset.state;
const changeKeyColor = (key) => {
  getKeyState(key);
  key.style = statuses[keyState]
};
const alertPlayer = (selector) => {
  $(document).ready(function (){
      $(selector).dialog();
  });
};
const checkAnswer = () => {
  userStats.setItem("guess-count", ++guessCount);
  let tryOrTries = guessCount === 1 ? " try" : " tries"
  if(guessWord.toLowerCase() === answerWord.toLowerCase()){
    document.querySelector("#you-win").textContent = "Winner! It took you " + guessCount + tryOrTries + " to figure out the word!"
    alertPlayer("#you-win")
    updateWinStreak()
  }else if(guessCount === 6){
    alertPlayer("#out-of-guesses")
    breakWinStreak();
  }
  guessWord = "";
 }
const submitGuess = () => {
  if(guessWord.length !== 5) return alertPlayer("#not-enough-characters");
  if(!targetWords.includes(guessWord.toLowerCase())) return alertPlayer("#not-an-eldrow-word")
  ...
  for(let i = 0; i < guessWord.length; i++){
    ...
    (function flipLoop(i, guessWord, guessBlocks) {
      setTimeout(function() {
        ...
        if (++i < guessWord.length ) flipLoop();
      }, i * 500)
...
buttons.forEach(button => {
  button.addEventListener("click", () => {
    if(button.dataset.key.length === 1){
      inputLetter(nextEmptyTile ,button.dataset.key)
